...
<div class="post-tab cf">
	<?php if($tab1name): ?><div class="tab1 tab-active"><?php echo $tab1name; ?></div><?php endif; ?>
	<?php if($tab2name): ?><div class="tab2 <?php if(!$tab1name) echo 'tab-active'; ?>"><?php echo $tab2name; ?></div><?php endif; ?>
	<?php if($tab3name): ?><div class="tab3"<?php if(!$tab1name) echo 'style="border-top:none;"'; ?>><?php echo $tab3name; ?></div><?php endif; ?>
	<?php if($tab4name): ?><div class="tab4"><?php echo $tab4name; ?></div><?php endif; ?>
</div>
<!--タブの中身-->
<?php if($tab1name) :?>
	<div class="post-tab__content tab1 tab-active">
		<?php get_template_part('parts/post-grid');//トップ記事一覧 ?>
	</div>
<?php endif; ?>
<?php $i = 2;/*tab2〜tab4を出力*/
while($i < 5){
	if( "${'is_tab'.$i.'tag'}") {//タグIDで一覧を取得
		$posts = get_posts(array(
		'posts_per_page' => $tab_cat_num,
		'tag__in' => "${'tab'.$i.'id'}" //タグIDを指定
		));
	} else {//カテゴリーIDで一覧を取得
		$posts = get_posts(array(
		'posts_per_page' => $tab_cat_num,
		'category' => "${'tab'.$i.'id'}" //カテゴリIDを指定
		));
	}
	if($posts):
		$tab_active = ((!$tab1name)&&($i==2)) ? ' tab-active' : null;//新着記事を表示しないときは、タブ2をアクティブに
		echo '<div class="post-tab__content tab'.$i.$tab_active.'">';
		if(is_sidelong()) { //(1)横長の場合
...
?>
		<div class="post-tab__more ct">
			<a class="raised main-bc strong" href="<?php echo "${'tab'.$i.'link'}"; ?>"><i class="fa fa-caret-right"></i> <span>もっと見る</span></a>
		</div>
<?php endif;
	echo '</div>';
	endif;
